...
#define RNOR (hz = KISS, iz = hz & 127, ( abs ( hz ) < kn[iz] ) ? hz * wn[iz] : nfix())
    ...
    class dZiggurat {
    public:
        dZiggurat(uint32_t seed=123456789) : jcong(380116160), jsr(123456789),
    ...
    class sZiggurat {
    public:
        sZiggurat(uint32_t seed=123456789) : jcong(380116160), jsr(123456789),
                                            w(521288629), z(362436069) {
            init();
            setSeed(seed);
        }
    ...
    private:
        ...
        void init() {               // called from ctor, could be in ctor
            ...
            fn[0] = 1.0;
            fn[127] = (float) (expf(- 0.5 * dn * dn));

            for (i = 126; 1 <= i; i--) {
                dn = sqrtf(- 2.0 * logf (vn / dn + expf (- 0.5 * dn * dn)));
                kn[i+1] = (uint32_t ) ((dn / tn ) * m1);
                tn = dn;
                fn[i] = (float) (expf( - 0.5 * dn * dn));
                wn[i] = (float) (dn / m1);
            }
            return;
        }
        ...
        inline float nfix(void) {
            ...
            for (;;) {
                ...
                //  0 < IZ, handle the wedges of other strips.
                if ( fn[iz] + UNI * ( fn[iz-1] - fn[iz] ) < expf (- 0.5 * x * x ) ) {
                    return x;
                }
                //  Initiate, try to exit the loop.
                hz = SHR3;      // note we still use SHR3; using KISS leads to degradation
